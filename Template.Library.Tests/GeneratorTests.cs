...
public sealed class GeneratorTests
{
    ...
    [Fact]
    public void RegistryListsTargetsInOrdinalOrder()
    {
        // Arrange
        const string source =
            """
            ...
            internal partial interface IGeneric<out T>
            {
                [CustomMethod]
                public static partial void Method();
            }
            ...
            {
                public const int Count = global::Test.Root.CustomMethodRegistry.Count;
            }
            """;

        // Act
        var registry = GeneratorTestHelper.GetRegistrySource(source, "Test.Root");

        // Assert
        Assert.Contains("namespace Test.Root;", registry, StringComparison.Ordinal);
        Assert.Contains("public const int Count = 3;", registry, StringComparison.Ordinal);
        var generic = registry.IndexOf("\"Test.IGeneric<T>.Method\"", StringComparison.Ordinal);
        var nested = registry.IndexOf("\"Test.Outer.Inner.Method\"", StringComparison.Ordinal);
        var target = registry.IndexOf("\"Test.Target.Method\"", StringComparison.Ordinal);
        Assert.True(generic >= 0);
        Assert.True(nested > generic);
        Assert.True(target > nested);
    }

    [Fact]
    public void RegistryIsNotGeneratedWithoutTarget()
    {
        // Arrange
        const string source =
            """
